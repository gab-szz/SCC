...
def open_main_window():
    class funcs():
        ...
        def add_cliente():
            ...
            cidade = cidadee.get()
            bd_scc.connect_bd("BD_SCC.db")
            cursor.execute(""" INSERT INTO clientes (nome_cliente, tel_cliente, cid_cliente)
            VALUES (?, ?, ?)""", (nome, telefone, cidade))
            bd_scc.disconnect_bd()
            
            tv.delete(*tv.get_children())
            bd_scc.connect_bd("BD_SCC.db")
            lista = cursor.execute("""SELECT cod, nome_cliente, tel_cliente, cid_cliente FROM clientes ORDER BY nome_cliente ASC;""")
            for i in lista:
...
# Botões
    ...
    novob = Button(main_window, text="Novo", bg='#339900',
               bd=2, fg="white", font=('verdana', 8, 'bold'), command=lambda: funcs().add_cliente())
